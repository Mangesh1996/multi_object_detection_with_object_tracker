    ...
    try:
        ...
        for img in sorted(os.listdir(os.path.join(os.getcwd(),path_img)),key=lambda f:int("".join(filter(str.isdigit,f)))):
            ...
            for cor in read:                
                ...
                h=round(float(cors[7]))
                new_cordinate=(x,y,w,h)
                center_points_cur_frame.append(new_cordinate)
            center_points_cur_frame_copy =center_points_cur_frame.copy()
            tracking_objects_copy = tracking_objects.copy()
            for object_id,pt2 in tracking_objects_copy.items():
                object_exists=False
        ...
        filename = exception_traceback.tb_frame.f_code.co_filename
        line_number = exception_traceback.tb_lineno
        print("Exceception ",exception_type)
        print("filename",filename)
        print("line Number",line_number)

read_src("person_image")
# face_detection_tracker("/home/diycam/Desktop/Desktop_Items/test/open_cv/opencv_work/save")